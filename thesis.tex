...
\newcommand*\nthkmean[1]{\overline{#1}}
\newcommand*\nthkres[1]{{#1}^{\prime}}
\newcommand*\thkmean[1]{\left\langle{#1}\right\rangle}
\newcommand*\thkres[1]{{#1}^{\#}}
 ...
 isopycnal coordinates, that is in the coordinate space $(x,y,\rho)$,
  ...
  where $z_{\rho} = \frac{\partial z}{\partial \rho}$ is defined as the
   ...
   so that the thickness equation becomes
    ...
    step further and proposes that eddies act to dissipate potential
    energy by diffusing the isopycnal thickness, hence the 
